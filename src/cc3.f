...
C*
C**************************************************************************
      REAL FUNCTION CC3(NSAM,IRAD,AMAG,SPEC,A3DF,
      ...
      DO 30 I=0,JC-1
          ...
          IF ((ITEMP.GE.MAXR12).AND.(ITEMP.LT.MAXR22)) THEN
      	    ...
      	    IF (JJ.LT.1) JJ=JJ+NSAM
      	      ...
      	      CTFR=CTFF(ID)
      	      CTFL=CTFF(ID+IC)
C            IF (IEWALD.NE.0) THEN 
C MW call ewald sphere corrected extraction
C              IF (IEWALD.LT.0) THEN
C                CTFR=CONJG(CTFR)
C                CTFL=CONJG(CTFL)
C              ENDIF
C              AINPO=EWALDEX(NSAM,IRAD,A3DF,SINCLUT,IPAD,
C     +                         I,J,DM,THETATR,CTFR,CTFL)
C            ELSE 
      	      X3=DM(1)*I+DM(4)*J
      	      Y3=DM(2)*I+DM(5)*J
      	      Z3=DM(3)*I+DM(6)*J
     ...
     +                              X3,Y3,Z3,SINCLUT,IPAD)
                ENDIF
C              ENDIF
              IF (REAL(CTFR+CONJG(CTFL)).LT.0.0)
     +        AINPO=-AINPO
