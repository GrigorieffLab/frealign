...
C
     ...
     +      (NXYZ(3).GT.0).AND.(NXYZ(3).LT.1.0E8)) THEN
          CFORM='S'
          GOTO 99
        ENDIF
C
...
C
     ...
     +      (I1(3).GT.0).AND.(I1(3).LT.1.0E8).AND.
...
C
C**************************************************************************
      SUBROUTINE IOPEN(CNAME,IFILE,CFORM,NX,NY,NZ,
     .                 CSTIN,ASYM,PSIZE,VX)
C**************************************************************************
      IMPLICIT NONE
C
      ...
      CHARACTER CDAT*24,ASYM*3,VX*15
      LOGICAL BS(IM)
C
      DATA BB/0/
      DATA MAPCRS/1,2,3/
...
C**************************************************************************
C
C     FREALIGN specific title
      TITLE='Frealign vX.XX - XX.XX.XX, output symmetry '
      CALL FDATE(CDAT)
      TITLE(11:25)=VX
      TITLE(44:46)=ASYM
      TITLE(47:48)='  '
      TITLE(49:72)=CDAT
C     Always use REAL for FREALIGN output files
      MODE=2
C
...
C
        WRITE(*,'(A12,3I11)')' NX, NY, NZ:',(NXYZ(I,IFILE),I=1,3)
...
C  This check seems to fail with EMAN-generated hed/img files (Chuck)
C        IF (REALTYPE1.NE.REALTYPE)
C     +    STOP ' ARCHITECTURE INCOMPATIBILITY'
C
...
C**************************************************************************
      ...
      CDAT(1:10)='    '//MONTH(DTVAL(2))//' '//CDATE(7:8)
      CDAT(11:16)=' '//CTIME(1:2)//':'//CTIME(3:4)
      CDAT(17:24)=':'//CTIME(5:6)//' '//CDATE(1:4)
      RETURN
      END
