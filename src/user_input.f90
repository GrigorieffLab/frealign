...
module UserInputs
    ...
    subroutine Init(self, wanted_program_name)
        ...
        if (this_program%running_interactively .and. this_program%reading_from_terminal) then
            allocate(character(len=defaults_filename_length) :: self%defaults_filename)
            write(self%defaults_filename, '(3a)')'.', trim(adjustl(wanted_program_name)), '.dff'

            !new defaults filename is same as above but with current after the dot.

            defaults_filename_length = defaults_filename_length + 7
            allocate(character(defaults_filename_length) :: self%new_defaults_filename)
            write(self%new_defaults_filename, '(3a)')'.current', trim(adjustl(wanted_program_name)), '.dff'
            ...
            open(unit=self%new_defaults_file_unit, file=self%new_defaults_filename, iostat=ios)
...
!    end function GetOptionFromUser
    ...
    function GetFilenameFromUser(self, question_to_ask, help_text_and_description, associated_keyword, default_value, &
        ...
        if (this_program%running_interactively) then
            ...
            call CopyString(user_supplied_entry, returned_value%value)
    ...
    subroutine AskQuestion(question_to_ask, current_default_value, user_supplied_entry)
        ...
        if (combined_length < user_input_max_len) then ! no need for wrapping, reasonably simple
            ...
            write(*, '(a)', advance='no') trim(question_to_ask)
            ...
            write(*,'(3a)', advance='no') ' [', trim(current_default_value), '] : '
        ...
        if (.not. this_program%reading_from_terminal) then
            write(*,"(a)") trim(adjustl(user_answer))
        endif
    ...
    subroutine UpdateDefaults(self)
        ...
        if (UnitIsOpen(self%defaults_file_unit) .and. UnitIsOpen(self%new_defaults_file_unit)) then
            ...
            do while (ios == 0)
                ...
                if ( ios == 0 ) then
                    ...
                    ! now we need to check if this value is in the new_defaults file, and if not, carry it over

                    call GetKeywordValueFromFile(self%new_defaults_filename, label, value2, value_was_found)
                    ! If we get here and ffound still equals false, then we need to write this value into the file
            ...
            ! the defaults file should now be copied over the the new_defaults_file.. so close and delete the defaults file

            if (UnitIsOpen(self%defaults_file_unit)) then
                close(self%defaults_file_unit, status='delete')
                call this_program%ReleaseUnit(self%defaults_file_unit)
            endif
            ...
            if (UnitIsOpen(self%new_defaults_file_unit)) then
                close(self%new_defaults_file_unit, status='delete')
                call this_program%ReleaseUnit(self%new_defaults_file_unit)
            endif
        endif
        ...
        if (this_program%running_interactively .and. this_program%reading_from_terminal) then
            ...
            if (ios .ne. 0) then
                write(*,'(a)') '**warning(UserInput::Init): failed to open new defaults file for writing', &
                           &trim(adjustl(self%new_defaults_filename))
            endif
        endif
    ...
    subroutine Destroy(self)
        ...
        if (UnitIsOpen(self%defaults_file_unit)) close(self%defaults_file_unit)
        if (UnitIsOpen(self%new_defaults_file_unit)) close(self%new_defaults_file_unit, status='delete')
